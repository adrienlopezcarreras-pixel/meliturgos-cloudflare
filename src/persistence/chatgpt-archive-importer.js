...
async function enrichExistingArchiveMessage(db, id, existingRow, message) {
  ...
  if (!attachmentBackfill && !contentBackfill) {
    return { changed: false, attachment_backfill: false, content_backfill: false };
  }
  ...
  await db.prepare('UPDATE archive_messages SET content=?, attachments_json=?, metadata=? WHERE id=?')
    .bind(
      contentBackfill ? incomingContent : existingContent,
