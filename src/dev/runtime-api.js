...
export function devRuntime(request,env){const u=new URL(request.url), p=u.pathname; if(!p.startsWith('/api/professor/dev')&&!p.startsWith('/api/dev-bridge')) return null;
 ...
 return (async()=>{const body=await request.json().catch(()=>({})); const repo=new D1DevJobRepository(env.DB), bridges=new D1BridgeRepository(env.DB);
  ...
  if(p==='/api/dev-bridge/result'&&request.method==='POST'){if(body.status==='READY_FOR_REVIEW'){body.result_json=body.result_json||body.result||((body.tests||body.diff_summary)?{steps:[],tests:body.tests||[],diff_summary:body.diff_summary||'NO_CHANGES'}:null);requireValue(body.result_json,'RESULT_REQUIRED',422);}return Response.json(await repo.update(body.job_id,body));}
