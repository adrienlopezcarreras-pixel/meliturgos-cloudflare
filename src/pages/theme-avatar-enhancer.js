import { THEME_VISUALS } from './theme-visual-assets.js';
import { getMelAvatarRoute } from './mel-avatar-assets.js';

const granadaBaroque = 'https://verite-interdite.fr/wp-content/uploads/2026/09/mel-bg-granada-hd-scaled.jpg?v=20260912-r4';
const futureAvatar = JSON.stringify(THEME_VISUALS.futureAvatar);

const THEME_AVATAR_SCRIPT = `<style id="mel-theme-decor-style">
/* Theme enhancer: self-contained original scenes, stable portraits, full-screen art. */
...
html[data-theme="classic"] .avatar{border:5px solid rgba(110,231,255,.72)!important;box-shadow:0 0 0 3px rgba(86,55,143,.65),0 0 38px rgba(80,220,255,.26)!important}
...
(function(){
  ...
  const avatars={
    classic:${futureAvatar},
    crusade:'${getMelAvatarRoute('crusade')}',
    religious:'${getMelAvatarRoute('religious')}',
    granada:'${getMelAvatarRoute('granada')}',
    aviation:'${getMelAvatarRoute('aviation')}',
    paladin:'${getMelAvatarRoute('paladin')}',
    amazon:'${getMelAvatarRoute('amazon')}'
  };
