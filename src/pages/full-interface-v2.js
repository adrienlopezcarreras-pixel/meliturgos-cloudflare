...
function speakMelFrench(text){try{if(!('speechSynthesis'in window)||!text)return;window.speechSynthesis.cancel();const u=new SpeechSynthesisUtterance(String(text));u.lang='fr-FR';u.rate=.98;u.pitch=1;u.volume=1;const voices=window.speechSynthesis.getVoices();const fr=voices.filter(v=>String(v.lang||'').toLowerCase().startsWith('fr'));const preferred=fr.find(v=>/france|fr-fr|hortense|denise|audrey|google français|microsoft.*french/i.test((v.name||'')+' '+(v.lang||'')))||fr[0];if(preferred)u.voice=preferred;window.speechSynthesis.speak(u)}catch{}}
async function sendChat(){const input=qs('#chatInput'),text=input.value.trim();if(!text)return;input.value='';addMsg('user',text);qs('#chatSend').disabled=true;qs('#chatStatus').textContent='Réflexion…';try{const d=await jfetch('/api/chat',{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({text,conversation_id:conversationId,device_id:deviceId})});const answer=d.text||d.response||'Réponse vide.';addMsg('mel',answer);speakMelFrench(answer);qs('#chatStatus').textContent=''}catch(e){addMsg('mel','Erreur : '+e.message);qs('#chatStatus').textContent=e.message}finally{qs('#chatSend').disabled=false;input.focus()}}
