...
const titles={overview:['Vue d’ensemble','État réel des briques principales.'],chat:['Conversation','Chat et capacités automatiques.'],skills:['Compétences','CapabilityBus et santé.'],roadmap:['Feuille de route','Toutes les étapes du projet MEL.'],multi:['Multi-IA','Council, .augmentio et état des lieux.'],work:['Work','Tâches et développement.'],memory:['Mémoire','Persistance, RAG, import et sauvegarde.'],computer:['Ordinateur','Écran, souris, clavier et applications autorisées.'],terminal:['Terminal MEL','Waveshare, appairage, firmware et état matériel.'],lora:['LoRA UNCENSORED','Kaggle gratuit, checkpoints continus, impact et passage AGENTIC.'],diagnostics:['Diagnostic','Contrôles réels avant évolution ou release.']};
let roadmapCache=null,roadmapCacheAt=0,capabilityCache=null,capabilityCacheAt=0;
const PANEL_TTL_MS=15000,panelLoadedAt=new Map(),getInflight=new Map();
async function jfetch(url,opts={}){
  const method=String(opts?.method||'GET').toUpperCase();
  if(method==='GET'){
    const existing=getInflight.get(url);
    if(existing)return existing;
    const task=(async()=>{const r=await fetch(url,opts),t=await r.text();let d;try{d=JSON.parse(t)}catch{throw Error('Réponse serveur invalide')}if(!r.ok)throw Error(d.error||d.code||('HTTP '+r.status));return d})().finally(()=>getInflight.delete(url));
...
function renderCapabilitySummary(caps){
  ...
  qs('#capSummary').textContent=caps.filter(x=>x.enabled!==false).length+' actives';
}
...
function show(name){qsa('.view').forEach(v=>v.classList.toggle('active',v.dataset.panel===name));qsa('#nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===name));qs('#viewTitle').textContent=titles[name][0];qs('#viewSubtitle').textContent=titles[name][1];loadPanel(name).catch(()=>{});}
qsa('#nav button').forEach(b=>b.onclick=()=>show(b.dataset.view));qsa('[data-jump]').forEach(b=>b.onclick=()=>show(b.dataset.jump));
...
qs('#chatSend').onclick=sendChat;qs('#chatInput').addEventListener('keydown',e=>{if(e.key==='Enter'&&!e.isComposing){e.preventDefault();sendChat()}});
...
async function loadChatCapabilities(force=false){const summary=qs('#chatCapSummary'),groups=qs('#chatCapGroups'),list=qs('#chatCapList');if(!summary||!groups||!list)return[];summary.textContent='Lecture de mes capacités…';try{const d=await loadCapabilitiesData(force);const caps=Array.isArray(d.capabilities)?d.capabilities:[];const active=caps.filter(x=>x.enabled!==false&&!['OFFLINE','BLOCKED','DISABLED'].includes(String(x.health||'').toUpperCase()));const by={};for(const x of active){const f=capFamily(x.id);by[f]=(by[f]||0)+1}summary.textContent=active.length+' capacité'+(active.length>1?'s':'')+' disponible'+(active.length>1?'s':'')+' sur '+caps.length+' enregistrée'+(caps.length>1?'s':'')+'.';groups.innerHTML=Object.entries(by).sort((a,b)=>b[1]-a[1]).map(([k,n])=>'<span class="cap-chip">'+esc(k)+' · '+n+'</span>').join('');list.innerHTML=caps.map(x=>'<div class="cap-help-item"><strong>'+esc(x.name||x.id)+'</strong><small>'+esc(x.description||x.id||'')+'</small><span class="cap-help-state">'+esc(x.enabled===false?'DÉSACTIVÉ':(x.health||'ENREGISTRÉ'))+'</span></div>').join('')||'<div class="muted">Aucune capacité enregistrée.</div>';return caps}catch(e){summary.textContent='Capacités indisponibles : '+e.message;groups.innerHTML='';list.textContent='Impossible de lire le CapabilityBus.';throw e}}
qs('#chatCapRefresh').onclick=()=>{panelLoadedAt.delete('chat');loadChatCapabilities(true).then(()=>panelLoadedAt.set('chat',Date.now())).catch(()=>{})};
async function loadSkills(force=false){const box=qs('#skillsList');box.textContent='Chargement…';try{const d=await loadCapabilitiesData(force);const caps=d.capabilities||[];renderCapabilitySummary(caps);box.innerHTML='';const fragment=document.createDocumentFragment();for(const skill of caps){const el=document.createElement('div');el.className='skill';el.innerHTML='<div class="skill-head"><div><strong>'+esc(skill.name||skill.id)+'</strong><div class="muted">'+esc(skill.description||'')+'</div></div>'+statusTag(skill.health)+'</div>';fragment.appendChild(el)}box.appendChild(fragment);if(!caps.length)box.textContent='Aucune compétence enregistrée.';return caps}catch(e){box.textContent='Indisponible : '+e.message;throw e}}
...
async function loadChatGPTImportStatus(){try{const d=await jfetch('/api/gen2/import/chatgpt-status');const state=qs('#chatgptServerState');if(state){state.textContent=d.status==='ONLINE'?'ACTIF':'INDISPONIBLE';state.className='tag '+(d.status==='ONLINE'?'good':'bad')}qs('#chatgptServerConversations').textContent=Number(d.conversations||0).toLocaleString('fr-FR');qs('#chatgptServerMessages').textContent=Number(d.messages||0).toLocaleString('fr-FR');qs('#chatgptServerCandidates').textContent=Number(d.memory_candidates||0).toLocaleString('fr-FR');qs('#chatgptServerUnsynced').textContent=Number(d.unsynced_messages||0).toLocaleString('fr-FR');qs('#chatgptServerLast').textContent=d.last_received?.title||d.last_received?.conversation_id||'—';qs('#chatgptServerMemoryStage').textContent=d.stages?.memory_candidate_extraction||'—';qs('#chatgptStatusOut').textContent=JSON.stringify(d,null,2);return d}catch(e){const state=qs('#chatgptServerState');if(state){state.textContent='ERREUR';state.className='tag bad'}qs('#chatgptStatusOut').textContent='Suivi import indisponible : '+e.message;return null}}
...
async function mobileAutonomyAction(button,url,body,label){
  const feedback=qs('#mobileAutonomyFeedback');
  if(!button)return;
  ...
  try{
    ...
    renderAutonomy(d?.state||d);
    await loadAutonomy();
    if(feedback)feedback.textContent=label+' : OK';
    return d;
  }catch(e){
    if(feedback)feedback.textContent=label+' : '+e.message;
    throw e;
  }finally{
...
function renderMobileActivity(state){
  const panel=qs('#mobileActivityPanel');
  if(!panel)return;
  const items=Array.isArray(state?.recent_activity)?state.recent_activity.slice(0,6):[];
...
if(mobileMax)mobileMax.onclick=()=>mobileAutonomyAction(mobileMax,'/api/gen2/autonomy/max',{enabled:true},'MAX 100%').catch(()=>{});
if(mobilePause)mobilePause.onclick=()=>mobileAutonomyAction(mobilePause,'/api/gen2/autonomy/pause',{reason:'owner-mobile-standby'},'Veille').catch(()=>{});
if(mobileStart)mobileStart.onclick=()=>mobileAutonomyAction(mobileStart,'/api/gen2/autonomy/tick',{},'Cycle MEL').catch(()=>{});
if(mobileResume)mobileResume.onclick=()=>mobileAutonomyAction(mobileResume,'/api/gen2/autonomy/resume',{},'Reprise').catch(()=>{});
if(mobileActivity)mobileActivity.onclick=async()=>{
  const panel=qs('#mobileActivityPanel');
  if(!panel)return;
  if(!panel.hidden){panel.hidden=true;return}
  panel.hidden=false;panel.textContent='Chargement de l’activité…';
  try{const d=await jfetch('/api/gen2/autonomy/state');renderMobileActivity(d)}catch(e){panel.textContent='Activité indisponible : '+e.message}
};
...
async function loadFreeLoraStatus(){
  ...
  try{
    ...
    const files=Array.isArray(d?.bundle?.files)?d.bundle.files:[];
    const bundleReady=d?.bundle?.ready===true;
    const training=d?.bundle?.training||null;
    const dataset=d?.bundle?.dataset||null;
    const registry=d?.bundle?.adapter_registry||null;
    const wf=d?.workflow||{};
    const trainWf=d?.training_workflow||{};
    const checkpoint=d?.checkpoint||null;
    const trainStatus=String(trainWf.status||'NEVER_RUN').toUpperCase();
    ...
    qs('#freeKaggleState').textContent=trainActive?'RUNNING':(trainStatus==='COMPLETED'?(trainConclusion||'COMPLETED'):trainStatus);
    qs('#freeKaggleCycle').textContent=visibleCycle==null?'—':'C'+String(visibleCycle).padStart(3,'0');
    const wfStatus=String(wf.status||'NEVER_RUN').toUpperCase();
    const wfConclusion=String(wf.conclusion||'').toUpperCase();
    const lora=d?.learning?.lora_status||{};
    const loraState=String(lora.state||'BLOCKED').toUpperCase();
    const lessonCount=Math.max(0,Number(d?.learning?.corrections_available_for_training||0));
    qs('#freeLoraLessons').textContent=lessonCount+'/50'+(lessonCount>=50?' · PRÊT':'');
    ...
    qs('#freeCheckpointStage').textContent=checkpoint?.tag?String(checkpoint.tag):(training?.stage?String(training.stage).toUpperCase():'AUCUN CHECKPOINT');
    qs('#freeCompatibleLoras').textContent=registry?.compatible_count!=null?Number(registry.compatible_count).toLocaleString('fr-FR'):'—';
    const bench=d?.learning?.benchmark_status||{};
    qs('#freeBenchmarkState').textContent=String(bench.status||'AUCUNE MESURE')+(bench.latest_score!=null?' · '+Math.round(Number(bench.latest_score)*1000)/10+'%':'');
    const impact=d?.learning?.impact||{}, cand=impact.candidate||{}, delta=impact.delta||{};
    const pctImpact=v=>v==null||!Number.isFinite(Number(v))?'—':(Math.round(Number(v)*1000)/10)+'%';
    qs('#freeImpactStage').textContent=impact.next_stage||'UNCENSORED_WAITING';
    ...
    const agenticReady=String(impact.next_stage||'')==='AGENTIC_READY';
    setLoraTag('#freeAgenticState',agenticReady?'AGENTIC_READY':'VERROUILLÉ',agenticReady?'good':'warn');
    ...
    if(trainActive)setLoraTag('#freeGpuState','RUNNING','good');
    ...
    else setLoraTag('#freeGpuState','À lancer','warn');
    setLoraTag('#freeHfState',checkpoint?.tag?'CHECKPOINT '+('C'+String(checkpointCycle).padStart(3,'0')):(bundleReady?'Bundle HF prêt':'En attente'),checkpoint?.tag||bundleReady?'good':'warn');

    let wfKind='warn',wfLabel='Jamais lancé';
    ...
    let runtimeKind='warn',runtimeLabel=loraState,runtimeDetail=lora.reason||'Aucun adaptateur actif prouvé.';
    ...
    qs('#freeLoraFiles').innerHTML=files.length?files.map(x=>(x.available?'✓ ':'○ ')+esc(x.name)).join('<br>'):'Aucun fichier détecté.';
    ...
    if(checkpoint?.tag||bundleReady)stage=2;
    ...
    qs('#freeLoraBar').style.width=percent+'%';
    qs('#freeLoraProgressPercent').textContent=percent+'%';
    qs('#freeLoraProgressText').textContent=percent===100?'Checkpoint UNCENSORED actif dans le preview':trainActive?'GPU Kaggle T4 en cours · cycle '+(visibleCycle==null?'?':visibleCycle):(checkpoint?.tag?'Checkpoint UNCENSORED conservé · benchmark/gate à suivre':bundleReady?'Bundle UNCENSORED prêt · promotion candidate à lancer':'En attente du prochain cycle Kaggle gratuit');
  }catch(e){
...
qs('#terminalPairCreate').onclick=async()=>{
  ...
  try{
    ...
    qs('#terminalPairNote').textContent='Code valable jusqu’à '+new Date(Number(d.expires_at)).toLocaleTimeString('fr-FR')+'. Saisis-le sur la page MEL-SETUP du terminal.';
  }catch(e){qs('#terminalOut').textContent='Erreur : '+e.message}
  finally{b.disabled=false}
};
qs('#terminalRefresh').onclick=()=>loadTerminal();

async function boot(){const tasks=await Promise.allSettled([loadCapabilitySummary(),loadRoadmapSummary(),codeCheck(),loadAutonomy()]);const ok=tasks.filter(x=>x.status==='fulfilled'&&x.value!==false).length,total=tasks.length;qs('#globalState').textContent=ok===total?'Système prêt':ok?'Partiellement prêt':'Diagnostic requis';qs('#globalDot').className='dot '+(ok===total?'good':ok?'warn':'bad');qs('#healthRows').innerHTML='<div class="status-row"><span>CapabilityBus</span><span>'+esc(qs('#capSummary').textContent)+'</span></div><div class="status-row"><span>Roadmap</span><span>'+esc(qs('#roadPercent').textContent)+'%</span></div><div class="status-row"><span>Code</span><span>'+esc(qs('#codeMetric').textContent)+'</span></div>'}
...
boot();
</script><script id="mel-control-center-runtime">
(()=>{
