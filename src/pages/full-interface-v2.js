...
<section class="view" data-panel="skills"><div class="section-title"><h2>Compétences</h2><p>État réel des capacités du CapabilityBus, vérifié à l’ouverture de cette page.</p></div><div class="roadmap-head" id="skillsHealthSummary"><div class="roadstat"><span class="muted">Actives</span><strong id="skillsHealthy">—</strong></div><div class="roadstat"><span class="muted">Protégées</span><strong id="skillsProtected">—</strong></div><div class="roadstat"><span class="muted">Dégradées</span><strong id="skillsDegraded">—</strong></div><div class="roadstat"><span class="muted">Non configurées</span><strong id="skillsUnavailable">—</strong></div></div><article class="card wide"><div class="actions" style="margin-top:0"><button id="refreshSkills">Actualiser la santé</button><span id="skillsProviderSummary" class="muted" role="status" aria-live="polite">Vérification des fournisseurs…</span></div><div id="skillsList" style="margin-top:12px">Chargement…</div></article></section>
...
function capabilityUsable(x){return x?.enabled!==false&&!['UNAVAILABLE','OFFLINE','BLOCKED','DISABLED'].includes(String(x?.health||'').toUpperCase())}
function capabilityHealthMeta(skill){
  const raw=skill?.enabled===false?'DISABLED':String(skill?.health||'UNKNOWN').toUpperCase();
  const labels={HEALTHY:'ACTIF',ONLINE:'ACTIF',PROTECTED:'PROTÉGÉ',DEGRADED:'DÉGRADÉ',UNAVAILABLE:'NON CONFIGURÉ',OFFLINE:'HORS LIGNE',DISABLED:'DÉSACTIVÉ'};
  const kind=['HEALTHY','ONLINE'].includes(raw)?'good':['UNAVAILABLE','OFFLINE','DISABLED'].includes(raw)?'bad':'warn';
  return{raw,label:labels[raw]||raw,kind};
}
function renderCapabilitySummary(caps){
  qs('#capCount').textContent=caps.length;
  qs('#capSummary').textContent=caps.filter(capabilityUsable).length+' actives';
}
...
function show(name){qsa('.view').forEach(v=>v.classList.toggle('active',v.dataset.panel===name));qsa('#nav button[data-view]').forEach(b=>b.classList.toggle('active',b.dataset.view===name));qs('#viewTitle').textContent=titles[name][0];qs('#viewSubtitle').textContent=titles[name][1];const menu=qs('#mobileMoreMenu'),more=qs('#mobileMoreNav');if(menu)menu.hidden=true;if(more)more.setAttribute('aria-expanded','false');loadPanel(name).catch(()=>{});}
...
modeTabs.forEach(tab=>tab.onclick=async()=>{
  const development=tab.dataset.mode==='development';
  modeTabs.forEach(x=>{const active=x===tab;x.classList.toggle('active',active);x.setAttribute('aria-selected',String(active));});
  ...
  if(development){await loadWork().catch(()=>{});panelLoadedAt.set('work',Date.now());}
});
...
async function loadSkills(force=false){
  ...
  try{
    ...
    for(const skill of caps){
      ...
      else if(meta.raw==='DEGRADED')counts.degraded++;
      else counts.unavailable++;
    }
    const set=(id,value)=>{const el=qs(id);if(el)el.textContent=value};
    set('#skillsHealthy',counts.healthy);set('#skillsProtected',counts.protected);set('#skillsDegraded',counts.degraded);set('#skillsUnavailable',counts.unavailable);
