...
(function(){
  ...
  function setVoice(text){voiceStatus.textContent=String(text||'')}
  setVoice('');
  document.querySelectorAll('.mel-motto,.mel-idle-status').forEach(function(node){node.remove()});
  function purgeLegacyCopy(){
    const banned=['Présente, attentive, prête à avancer avec toi.','Touchez le visage de MEL pour parler · le texte reste toujours disponible','MEL veille et prie en silence.'];
    document.querySelectorAll('body *').forEach(function(node){if(node.children.length===0&&banned.includes((node.textContent||'').trim()))node.remove()});
  }
  purgeLegacyCopy();
  ...
  async function startVoice(){
    ...
    try{
      ...
      recorder.onstop=async function(){
        ...
        }catch(error){transcriptionUnavailable=true;console.warn('MEL voice deferred:',error);setVoice('Voix optionnelle · continue avec le texte');input.focus()}
      };
      recorder.start();
