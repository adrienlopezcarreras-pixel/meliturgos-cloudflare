...
const PLAIN_STYLE='MODE COMPLET : réponds en français normal, moderne, direct et professionnel. Tutoie Adrien simplement. Aucun registre médiéval, épique, mystique ou théâtral. Pour tout avancement technique, distingue strictement projet, action réellement lancée, résultat vérifié et preuve disponible. Ne prétends pas coder, tester, corriger ou déployer sans preuve runtime. Réponds maintenant sans commenter cette consigne :\\n\\n';
...
function install(){const multi=document.querySelector('.view[data-panel="multi"]');if(!multi||document.getElementById('mentorRoomCanonical'))return false;['mentorRoom','mentorRoomV4','melStatusPanelV5','melReaderToolsV5'].forEach(function(id){const n=document.getElementById(id);if(n)n.remove()});const section=multi.querySelector('.section-head');const status=document.createElement('div');status.id='melCanonicalStatus';status.innerHTML='<div class="status-actions"><button id="melCanonicalStatusBtn" type="button">Statut MEL</button><button id="melCanonicalMentorBtn" type="button">Conseil Mentor</button><button id="melCanonicalNextBtn" type="button">Prochaine tâche</button></div><div class="status-copy"><div id="melCanonicalStatusText" class="status-line">État vérifiable à la demande. Le Mentor gratuit utilise seulement le pool zéro-euro autorisé.</div><div id="melCanonicalEvidence" class="status-evidence"></div></div>';const room=document.createElement('article');room.id='mentorRoomCanonical';room.className='card wide mentor-room';room.innerHTML='<div class="mentor-room-head"><div><h3>Salon Adrien · MEL · Mentor gratuit</h3><div class="mentor-note">Un seul salon canonique. Pas d’API OpenAI payante. Le Mentor gratuit relit MEL avec les modèles Workers AI admis par le Zero-Euro Governor ; sinon il bascule sur un conseil local déterministe.</div></div><span class="mentor-chip">0 € ajouté</span></div><div class="mentor-tools"><button id="melReaderLastCanonical" type="button">Dernière réponse</button><button id="melReaderBottomCanonical" type="button">Fin du chat</button><button id="melReaderLargeCanonical" type="button">Grande lecture</button><span class="mentor-hint">↕ fenêtre redimensionnable · F5 revient au dernier onglet</span></div><div id="mentorRoomLogCanonical" class="mentor-log"></div><div class="mentor-compose"><select id="mentorRoomTargetCanonical"><option value="all">@tous · MEL + Mentor gratuit</option><option value="mel">@MEL</option><option value="mentor">@Mentor gratuit</option></select><textarea id="mentorRoomInputCanonical" placeholder="Écris normalement ici…"></textarea><button id="mentorRoomSendCanonical" class="primary" type="button">Envoyer</button></div><div id="mentorRouteCanonical" class="mentor-route-state">Routage : MEL + Mentor gratuit</div><div class="mentor-hint">Le Mentor gratuit n’est pas ChatGPT : c’est un reviewer zéro-euro local/Workers AI. Il doit conseiller MEL sans créer une seconde branche de développement.</div>';
...
const log=document.getElementById('mentorRoomLogCanonical');const savedH=Number(read(HEIGHT_KEY));if(Number.isFinite(savedH)&&savedH>=330&&savedH<=1200)log.style.height=savedH+'px';if('ResizeObserver'in window)new ResizeObserver(function(){const h=Math.round(log.getBoundingClientRect().height);if(h>=330)store(HEIGHT_KEY,h)}).observe(log);
document.getElementById('melCanonicalStatusBtn').addEventListener('click',statusMel);document.getElementById('melCanonicalMentorBtn').addEventListener('click',mentorReview);document.getElementById('melCanonicalNextBtn').addEventListener('click',loadNext);document.getElementById('melReaderLastCanonical').addEventListener('click',jumpLast);document.getElementById('melReaderBottomCanonical').addEventListener('click',function(){jumpBottom(true)});document.getElementById('melReaderLargeCanonical').addEventListener('click',function(){const h=Math.min(Math.max(window.innerHeight*.74,560),960);log.style.height=Math.round(h)+'px';store(HEIGHT_KEY,Math.round(h));jumpBottom(true)});document.getElementById('mentorRoomSendCanonical').addEventListener('click',send);document.getElementById('mentorRoomInputCanonical').addEventListener('keydown',function(e){if(e.key==='Enter'&&!e.shiftKey&&!e.isComposing){e.preventDefault();send()}});document.querySelectorAll('.nav button[data-view]').forEach(function(b){b.addEventListener('click',function(){store(PANEL_KEY,b.dataset.view)})});renderRoom();const panel=read(PANEL_KEY)||'multi';setTimeout(function(){activatePanel(panel);if(panel==='multi')jumpBottom(false)},120);return true}
