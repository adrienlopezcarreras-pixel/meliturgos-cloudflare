...
<article id="mentorRoomCanonical" class="card wide mentor-room">
  ...
  <details class="mentor-details"><summary>Contrôle zéro-euro</summary><p>Le Mentor utilise /api/gen2/mentor/chat. Sans les deux confirmations de gratuité côté serveur, il reste local et n’effectue aucune inférence externe.</p></details>
</article>`;
...
async function statusMel(){const out=document.getElementById('melCanonicalStatusText'),ev=document.getElementById('melCanonicalEvidence'),btn=document.getElementById('melStatusBtnV5');if(!out||!ev||!btn)return;btn.disabled=true;btn.textContent='Vérification…';try{const results=await Promise.allSettled([askMel(STATUS_PROMPT),getJson('/api/gen2/code/self-check'),getWorkState(),askFreeMentor('Réponds uniquement par ton état de facturation et ton mode de contrôle.','')]);out.textContent=results[0].status==='fulfilled'?results[0].value:'Réponse MEL indisponible.';const bits=[];if(results[1].status==='fulfilled'&&results[1].value&&results[1].value.ok)bits.push('Dépôt '+String(results[1].value.branch||'?')+' @ '+String(results[1].value.sha||'?'));if(results[2].status==='fulfilled'){const s=results[2].value;bits.push(s.job?'Job '+String(s.job.id||'?')+' · '+String(s.job.status||'?'):'Aucun job actif vérifié');if(s.next)bits.push('Prochaine '+String(s.next.id||'?'))}if(results[3].status==='fulfilled')bits.push(results[3].value.meta);ev.textContent=bits.join(' · ')||'Aucune preuve runtime disponible.'}finally{btn.disabled=false;btn.textContent='Statut MEL'}}
