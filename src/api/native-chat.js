...
export async function handleNativeChat(request, env) {
  ...
  const system = [
    ...
    'CONTRÔLE ORDINATEUR : les capacités computer.status, computer.quick et computer.execute désignent le compagnon Windows explicitement appairé par Adrien. Une commande computer.quick SUCCEEDED signifie que l’action a été mise en file pour le compagnon ; ne prétends pas qu’elle est déjà terminée tant que le résultat du compagnon ne le prouve pas. Les commandes sensibles ne sont approuvées que lorsqu’elles proviennent explicitement de la demande courante ou de l’onglet Ordinateur.',
    memoryWrite.stored
