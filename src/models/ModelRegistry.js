...
export class ModelRegistry {
  ...
  getFallbackChain(primaryId, limit = 2) { const primary = this.get(primaryId); const capabilitySet = new Set((primary?.capabilities || []).map(cap => String(cap).toUpperCase())); const candidates = this.list().filter(model => model.id !== primaryId && model.enabled && model.health !== 'UNAVAILABLE').map(model => ({ model, overlap: (model.capabilities || []).reduce((score, cap) => score + (capabilitySet.has(String(cap).toUpperCase()) ? 1 : 0), 0) })).sort((a, b) => b.overlap - a.overlap || (b.model.priority || 0) - (a.model.priority || 0)).map(entry => entry.model); return candidates.slice(0, Math.max(0, Number(limit) || 0)); }
