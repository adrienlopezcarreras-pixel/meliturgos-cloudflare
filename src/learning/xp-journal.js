 ...
 * monotone and durable proof artifacts are attached.
 */
export async function recordLearningXpCheckpoint({ memory, report = {}, date = '', reason = '', artifacts = [], source_sha = null, now = () => new Date() } = {}) {
  ...
  await memory.remember({
    ...
    lesson: awarded
      ? `XP verified ${previousXp} -> ${progress.xp} (+${measuredDelta}) with durable evidence.`
      : 'XP snapshot recorded without award: no strictly proven gain with durable evidence.',
    evidence,
