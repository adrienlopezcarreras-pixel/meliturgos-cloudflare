...
async function projectExperienceStatus(db) {
  const base = { count: 0, source: PROJECT_EXPERIENCE_SOURCE, kind: 'lesson', available: false };
  if (!db || typeof db.prepare !== 'function') return base;
  try {
    const row = await db.prepare('SELECT COUNT(*) AS count FROM memories WHERE source = ?')
      .bind(PROJECT_EXPERIENCE_SOURCE)
      .first();
    return { ...base, count: finiteNonNegativeInt(row?.count, 0), available: true };
  } catch {
    return base;
  }
}
 ...
 * Canonical XP comes from the verified journal when present.
 * Project experience is reported separately and never converted into XP.
 */
