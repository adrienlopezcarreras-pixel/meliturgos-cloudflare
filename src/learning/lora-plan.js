...
const MAX_CLOUDFLARE_ADAPTER_BYTES = 100_000_000;
...
export function isSupportedCloudflareLoraRuntimeModel(model) {
  return CLOUDFLARE_LORA_RUNTIME_MODELS.includes(String(model || '').trim());
}

export function isSupportedCloudflareLoraPair(baseModel, runtimeModel) {
  return CLOUDFLARE_LORA_MODEL_PAIRS[String(baseModel || '').trim()] === String(runtimeModel || '').trim();
}
...
export function createLoraTrainingPlan({
  ...
  const normalizedRuntime = safeId(runtime, 'RUNTIME');
  const normalizedRuntimeModel = safeId(runtime_model, 'RUNTIME_MODEL');
  const normalizedBaseModel = safeId(base_model, 'BASE_MODEL');
  const runtimeModelSupported = normalizedRuntime === 'cloudflare-workers-ai'
    && isSupportedCloudflareLoraPair(normalizedBaseModel, normalizedRuntimeModel);
  ...
  return {
    ...
    min_measured_gain: Math.max(0, Math.min(1, finiteNumber(min_measured_gain, 0.02))),
    ...
    readiness: {
      ...
      runtime_model_supported: runtimeModelSupported,
      cloudflare_inference_compatible: cloudflareCompatible,
      cloudflare_requirements: {
        quantization: 'none',
        max_rank: 32,
        max_adapter_bytes: MAX_CLOUDFLARE_ADAPTER_BYTES,
        expected_files: ['adapter_config.json', 'adapter_model.safetensors'],
        supported_runtime_models: [...CLOUDFLARE_LORA_RUNTIME_MODELS],
      },
    },
  };
}
...
export function assertAdapterArtifact(artifact = {}) {
  ...
  const runtimeModel = safeId(artifact.runtime_model, 'RUNTIME_MODEL');
  const runtime = safeId(artifact.runtime || 'cloudflare-workers-ai', 'RUNTIME');
  const format = bounded(artifact.format || 'safetensors', 80);
  const sizeBytes = Math.trunc(finiteNumber(artifact.size_bytes, 0));
  const rank = Math.trunc(finiteNumber(artifact.rank, 0));
  ...
  if (runtime !== 'cloudflare-workers-ai' || !isSupportedCloudflareLoraPair(baseModel, runtimeModel)) {
  ...
  return {
    ...
    rank,
    format,
    uri: bounded(artifact.uri, 2000) || null,
    training_manifest_digest: artifact.training_manifest_digest ? safeId(artifact.training_manifest_digest, 'TRAINING_MANIFEST_DIGEST') : null,
  };
}

export function assertAdapterActivationEvidence({ plan, artifact, baseline, candidate } = {}) {
  if (!plan?.readiness?.ready_for_training || !plan?.readiness?.benchmark_required_before_activation) {
    throw Object.assign(new Error('LORA_PLAN_NOT_ACTIVATABLE'), { code: 'LORA_PLAN_NOT_ACTIVATABLE' });
  }
  if (!plan?.readiness?.cloudflare_inference_compatible || !isSupportedCloudflareLoraPair(plan?.base_model, plan?.runtime_model)) {
    throw Object.assign(new Error('LORA_RUNTIME_INCOMPATIBLE'), { code: 'LORA_RUNTIME_INCOMPATIBLE' });
  }
  ...
  if (checked.runtime_model !== plan.runtime_model || checked.runtime !== plan.runtime) {
    throw Object.assign(new Error('LORA_RUNTIME_INCOMPATIBLE'), { code: 'LORA_RUNTIME_INCOMPATIBLE' });
  }
  if (checked.rank !== plan.rank) throw Object.assign(new Error('LORA_RANK_MISMATCH'), { code: 'LORA_RANK_MISMATCH' });
  if (checked.training_manifest_digest && checked.training_manifest_digest !== plan.dataset_digest) {
    throw Object.assign(new Error('LORA_TRAINING_MANIFEST_MISMATCH'), { code: 'LORA_TRAINING_MANIFEST_MISMATCH' });
  }
  const baseOverall = Number(baseline?.overall);
  const candidateOverall = Number(candidate?.overall);
  if (!Number.isFinite(baseOverall) || !Number.isFinite(candidateOverall)) {
    throw Object.assign(new Error('LORA_BENCHMARK_REQUIRED'), { code: 'LORA_BENCHMARK_REQUIRED' });
  }
  const baseSuite = String(baseline?.suite_digest || baseline?.metadata?.suite_digest || '').trim();
  const candidateSuite = String(candidate?.suite_digest || candidate?.metadata?.suite_digest || '').trim();
  if (!baseSuite || !candidateSuite || baseSuite !== candidateSuite) {
    throw Object.assign(new Error('LORA_BENCHMARK_SUITE_MISMATCH'), { code: 'LORA_BENCHMARK_SUITE_MISMATCH' });
  }
  const gain = candidateOverall - baseOverall;
  const minimumGain = Number.isFinite(Number(plan.min_measured_gain)) ? Number(plan.min_measured_gain) : 0.02;
  if (gain < minimumGain) {
    throw Object.assign(new Error('LORA_MEASURED_GAIN_INSUFFICIENT'), { code: 'LORA_MEASURED_GAIN_INSUFFICIENT', gain, minimumGain });
  }
...
export const loraPolicy = Object.freeze({
  ...
  maximum_cloudflare_adapter_bytes: MAX_CLOUDFLARE_ADAPTER_BYTES,
  preferred_target_modules: ['q_proj', 'v_proj'],
  activation_requires_benchmark: true,
  activation_requires_measured_gain: true,
  activation_requires_no_major_regression: true,
  activation_requires_exact_base_model_match: true,
  activation_requires_runtime_model_match: true,
