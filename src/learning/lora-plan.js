...
export function assertAdapterApprovalForArtifact({plan,artifact,approval}={}){
  ...
  const approvalId=benchmarkField(approval,'approval_id');
  if(!approvalArtifactDigest||!approvalManifestDigest||!approvalDatasetDigest||!approvalFinetuneId||!approvalArtifactId||!approvalId)throw Object.assign(new Error('LORA_ARTIFACT_APPROVAL_PROVENANCE_REQUIRED'),{code:'LORA_ARTIFACT_APPROVAL_PROVENANCE_REQUIRED'});
  ...
  return {approved:true,approval_id:safeId(approvalId,'APPROVAL_ID'),artifact_id:checked.id,artifact_digest:checked.digest,finetune_id:checked.finetune_id,dataset_digest:plan.dataset_digest,training_manifest_digest:planManifestDigest,approved_at:bounded(approval.approved_at,120)||null};
}
...
export function assertAdapterActivationEvidence({plan,artifact,approval,baseline,candidate}={}){
  ...
  const candidateApprovalId=benchmarkField(candidate,'approval_id');
  if(!candidateArtifactDigest||!candidateManifestDigest||!candidateDatasetDigest||!candidateApprovalId)throw Object.assign(new Error('LORA_BENCHMARK_PROVENANCE_REQUIRED'),{code:'LORA_BENCHMARK_PROVENANCE_REQUIRED'});
  ...
  if(candidateApprovalId!==checkedApproval.approval_id)throw Object.assign(new Error('LORA_BENCHMARK_APPROVAL_MISMATCH'),{code:'LORA_BENCHMARK_APPROVAL_MISMATCH'});
  ...
  return {artifact:checked,approval:checkedApproval,measured_gain:gain,minimum_gain:minimumGain,suite_digest:baseSuite,dataset_digest:plan.dataset_digest,training_manifest_digest:planManifestDigest,benchmark_artifact_digest:candidateArtifactDigest,benchmark_approval_id:candidateApprovalId};
}
