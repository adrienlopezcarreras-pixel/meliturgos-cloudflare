...
function sensitiveApprovals(session,steps){const set=new Set(["keyboard.type","app.open","clipboard.read","clipboard.write"]);return steps.filter(x=>set.has(String(x.action||""))).map(x=>({approved:true,session_id:session,step_id:String(x.id),action:String(x.action)}))}
...
async function queuePlan(db,device,{session_id,steps,approve_sensitive=false,allowed_origins=[]}={}){
 ...
 const plan=evaluateComputerUsePlan({session_id:session,owner_halt:false,device:{id:device.id,capabilities:device.capabilities},sandbox:{allowed_apps:device.allowed_apps,allowed_origins:Array.isArray(allowed_origins)?allowed_origins:[],max_steps:20},approvals,steps:normalized});
