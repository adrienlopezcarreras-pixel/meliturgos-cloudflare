...
function safePath(value = '') {
  ...
  if (path.startsWith('/') || path.includes('..') || path.includes('\\') || DENIED_PATH.test(path)) {
    throw Object.assign(new Error('CODE_PATH_DENIED'), { code: 'CODE_PATH_DENIED' });
  }
  return path;
}
...
function headers(token) {
  const h = {
    ...
    'user-agent': 'meliturgos-code-reader'
  };
  if (token) h.authorization = `Bearer ${token}`;
  return h;
}

function githubError(response, code) {
  const error = new Error(`${code}_${response.status}`);
  error.code = code;
...
export function registerGitHubCodeCapabilities(bus, options = {}) {
  ...
  bus.discover({
    id: 'code.read', name: 'GitHub code read', category: 'development', version: '1.0.0', provider: 'github',
...
export const githubCodeLimits = Object.freeze({ maxFileBytes: MAX_FILE_BYTES, maxSearchFiles: MAX_SEARCH_FILES, maxMatches: MAX_MATCHES });
