...
export const ROADMAP_REGISTRY_REVISION = '2026-09-25.24';
...
export const MASTER_ROADMAP = Object.freeze([
  ...
  phase('P02', 'Conversation, contexte et continuité', [
    ...
    item('MEL-CONTEXT-03', 'Open loops: reprendre automatiquement les travaux inachevés', 'PARTIAL', 'Persistance D1 owner-scoped, scheduler openloop.resume, leases, priorités et retry borné sont intégrés. Le lien durable Conversation -> Plan/Task -> Work -> Open Loop est maintenant implémenté: conversation_id persiste dans les plans et Work DAG, les checkpoints suivent work.run, la matérialisation transfère la boucle Plan vers Work, COMPLETED ferme la boucle et BLOCKED reste en attente passive sans retry infini. Preuve locale ciblée: 22/22 tests verts. Reste avant DONE_VERIFIED: CI exacte, merge, déploiement production et smoke runtime.', 'P1'),
  ...
  phase('P03', 'Mémoire personnelle et connaissance', [
    ...
    item('GEN2-12', 'Timeline personnelle', 'IN_PROGRESS', 'Timeline D1 durable et CapabilityBus fusionnés par PR #350 avec preuve read-only timeline.list. Reste: déployer le main qui contient #350 et obtenir le smoke production vert avant DONE_VERIFIED.', 'P1'),
    item('GEN2-13', 'Projects / Decisions', 'IN_PROGRESS', 'Projects/Decisions/Lessons D1 durables et CapabilityBus fusionnés par PR #350 avec preuve read-only project.list. Reste: déployer le main qui contient #350 et obtenir le smoke production vert avant DONE_VERIFIED.', 'P1'),
  ...
  phase('P06', 'Module Lab, évolution et apprentissage', [
    ...
    item('MEL-EVOL-05', 'Skill Registry durable', 'IN_PROGRESS', 'Persistance D1 durable et câblage runtime fusionnés par PR #330 puis #356: snapshots versionnés, activation/rollback, checksum SHA-256, validation de schéma, isolation par registry_key et restauration après redémarrage. Reste: persister un vrai Skill Registry sur D1 production puis prouver restauration et rollback runtime avant DONE_VERIFIED.', 'P1'),
  ...
  phase('P07', 'Work, agents et automatisations', [
    ...
    item('GEN2-39', 'Agents / automations', 'IN_PROGRESS', 'Registre agents/politiques owner-scoped présent sur main et runner Agent→Work gardé fusionné par PR #361. Reste: réconcilier la politique durable complète sur le main courant, brancher l’orchestration Council/Work et obtenir une preuve runtime production avant DONE_VERIFIED.', 'P1'),
    item('GEN2-40', 'Event Bus idempotent / follow-ups', 'IN_PROGRESS', 'Event Bus D1 durable et CapabilityBus fusionnés par PR #350 avec preuve read-only event.list. Reste: déployer le main qui contient #350 et obtenir le smoke production vert avant DONE_VERIFIED.', 'P1'),
  ...
  phase('P10', 'Téléphone, PC et appareils', [
    ...
    item('GEN2-59', 'Build et release Windows', 'IN_PROGRESS', 'Candidate exact-SHA Windows présente en PR #344 (build/sign/verify + CI contract). La branche doit être réconciliée sur main, passer les gates, être fusionnée puis validée sur Windows réel avant DONE_VERIFIED.', 'P2'),
