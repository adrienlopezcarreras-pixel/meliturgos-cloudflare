...
export const MASTER_ROADMAP = Object.freeze([
  ...
  phase('P02', 'Conversation, contexte et continuité', [
    ...
    item('MEL-CONTEXT-03', 'Open loops: reprendre automatiquement les travaux inachevés', 'IN_PROGRESS', 'Store D1 + leases + reprise idempotente déjà présents; intégration CapabilityBus ajoutée pour capturer les boucles conversation↔task, lister les reprises dues et relancer un Work DAG via work.run sans double exécution. Reste avant DONE_VERIFIED: brancher automatiquement les événements Work/Conversation au captureur et prouver un cycle de reprise production.', 'P1'),
  ...
  phase('P08', 'Connecteurs et web', [
    ...
    item('GEN2-37', 'Web / recherche', 'PARTIAL', 'Recherche sourcée + qualité de provenance des sources intégrées. Candidat de fermeture: seed_urls officiel exposé par l’API avec scope release strict, test de refus hors scope et preuve production obligatoire sur une source Cloudflare directe HIGH_PROVENANCE avec provenance complète. Reste: CI exacte, merge puis release/smoke live avant DONE_VERIFIED.', 'P1'),
  ...
  phase('P11', 'Sécurité, audit et gouvernance', [
    ...
    item('MEL-SEC-03', 'Supply-chain / d�pendances / CI', 'DONE_VERIFIED', 'SBOM runtime, lockfile/registry gate et attestation exacte commit/lockfile/SBOM/audit sont actifs. Toute promotion exige release_eligible=true et �choue ferm�e si le registre d audit est indisponible. Preuves: PR #317, tests cibl�s 15/15, CI 19/19, d�ploiement production 36150649958 enti�rement vert avec gate supply-chain, tests complets, preuve post-d�ploiement et HTTP final.', 'P1'),
  ...
  phase('P12', 'Résilience, sauvegarde et indépendance', [
    ...
    item('GEN2-48', 'Restore / disaster recovery', 'DONE_VERIFIED', 'Drill runtime isol� de la derni�re sauvegarde syst�me via CapabilityBus: snapshot v�rifi�, reconstruction logique D1 en m�moire, revalidation R2/runtime, teardown garanti et activation production interdite. Preuves: PR #297 merg�e et smoke production GEN2-48 pass� dans le run 36150649958.', 'P1'),
  ...
  phase('P15', 'Release, migration et maturité finale', [
    ...
    item('GEN2-55', 'Data integrity / final maturity tests', 'IN_PROGRESS', 'system.integrity + system.maturity sont des audits read-only born�s couvrant migrations/tables/r�f�rences/JSON/coh�rence temporelle, CapabilityBus et structure roadmap. Le workflow de release ex�cute d�sormais les deux audits sur D1 production et v�rifie leurs invariants sans r�paration automatique ni appel r�seau. CI + d�ploiement live requis avant DONE_VERIFIED.', 'P1'),
