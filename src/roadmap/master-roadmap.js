...
export const MASTER_ROADMAP = Object.freeze([
  ...
  phase('P03', 'Mémoire personnelle et connaissance', [
    ...
    item('GEN2-11', 'Knowledge Graph', 'DONE_VERIFIED', 'Contrat Knowledge Graph rejoué sur le main du 26/09/2026: entités typées avec source et confiance, relations sourcées, requête de voisinage et supersession explicite d’une décision vers sa remplaçante. Les validations rejettent les types inconnus, sources absentes et confiances invalides. Socle canonique: src/memory/knowledge-graph.js; origine historique: commit 0221384747c09ed59f66c032c534a4c82fcf2745.', 'P1'),
    ...
    item('MEL-MEM-04', 'Complétude des archives ChatGPT récupérables', 'DONE_VERIFIED', 'Clôturé par décision owner le 22/09/2026 : l’archive serveur déjà importée est conservée et l’import manuel reste disponible; le Collector Firefox est retiré du parcours cible et aucune preuve Collector supplémentaire n’est requise. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P0'),
  ...
  phase('P04', 'Capability Bus, outils et accès au code', [
    ...
    item('GEN2-15', 'Plugin SDK', 'DONE_VERIFIED', 'Certification production acquise le 26/09/2026 sur SHA exact 6a565654ff38401f064be06872565b09e8b8f41f, run 36234937611: cycle D1 candidate→tested→active sur deux versions, remplacement actif, rollback vers 1.0.0, recréation adapter/store et relecture persistante. Le drill plugin-sdk-proof a été exécuté deux fois avec statut GEN2_15_PRODUCTION_D1_VERIFIED et replay_safe=true.', 'P1'),
    item('GEN2-50', 'Compatibilité MCP', 'DONE_VERIFIED', 'Implémentation livrée par PR #26: adaptateur MCP natif sur CapabilityBus, négociation moderne/legacy, tools/list + tools/call et compatibilité écosystème. Présent dans le main exact déployé e749551d6c2786c225d5194a5a3556e4f47c84fd; conserver les tests de compatibilité. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P2')
  ]),
  ...
  phase('P06', 'Module Lab, évolution et apprentissage', [
    ...
    item('MEL-EVOL-04', 'EVOLUTION_LEDGER immuable et explicable', 'DONE_VERIFIED', 'Certification production acquise le 26/09/2026 sur SHA exact 6a565654ff38401f064be06872565b09e8b8f41f, run 36234937611: deux événements de preuve exact-SHA écrits dans D1, chaîne previous_hash/entry_hash vérifiée, relecture ciblée et vérification globale du ledger. Le drill evolution-ledger-proof a été exécuté deux fois avec statut MEL_EVOL_04_PRODUCTION_LEDGER_VERIFIED et replay_safe=true.', 'P1'),
    ...
    item('MEL-EVOL-06', 'Fine-tuning / LoRA open-weight continu', 'DONE', 'Pipeline continu open-weight fusionné: heartbeat, Kaggle GPU gratuit, checkpoints immuables, chaîne UNCENSORED puis AGENTIC, collector dispatch-only, garde zéro coût et benchmark isolé. Le run live 36229951853 a validé auth Kaggle, quota, shard, payload privé, soumission T4 et métadonnées GPU puis attend la fin du cycle. Checkpoint final + benchmark canonique requis avant DONE_VERIFIED.', 'P0'),
    item('GEN2-18', 'Self Healing contrôlé', 'DONE_VERIFIED', 'Implémentation candidate-only contrôlée fusionnée par PR #394; rollback/réparation restent bornés et non autonomes en production. Gates self-healing-controlled-ci 36196820120 et gen2-17-multi-cycle-ci 36196820150 verts sur le SHA du PR. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P2'),
  ...
  phase('P07', 'Work, agents et automatisations', [
    ...
    item('GEN2-39', 'Agents / automations', 'DONE_VERIFIED', 'Certification production acquise le 26/09/2026 sur SHA exact 6a565654ff38401f064be06872565b09e8b8f41f, run 36234937611: agent owner-scoped matérialisé en D1, autorisation READ, rejeu idempotent dédupliqué, complétion puis relecture après recréation des adapters. Le drill agent-automation-proof a été exécuté deux fois avec statut GEN2_39_PRODUCTION_D1_VERIFIED et persisted_across_adapter_recreation=true.', 'P1'),
    ...
    item('GEN2-41', 'Notifications', 'DONE_VERIFIED', 'Service durable owner-scoped + store D1 fusionnés sur main; gate gen2-41-notifications-ci vert sur le run 36196457534. Conserver la persistance et étendre les canaux/compagnons sans réouvrir le socle. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P2'),
  ...
  phase('P08', 'Connecteurs et web', [
    item('GEN2-32', 'Connector SDK', 'DONE_VERIFIED', 'Validation réelle acquise le 26/09/2026: suite contractuelle Connector SDK officielle verte dans le run 36272469638; provider Cloudflare réel passé CONFIGURED -> CONNECTED avec health CHECKED et auth vérifiée via CLOUDFLARE_API_TOKEN, sans mutation ni exposition de secret. Provider GitHub réel repassé sur le main courant via le token utilisateur gh du PC: CONFIGURED -> CONNECTED, health CHECKED, auth vérifiée, 3 capacités déclarées, zéro mutation. Le token GitHub automatique Actions est un token installation et n\'est volontairement pas accepté par la sonde /user; les OAuth/permissions propres aux autres fournisseurs restent suivis séparément.', 'P1'),
    item('GEN2-33', 'Gmail / Google', 'BLOCKED_HUMAN', 'Runtime OAuth Google persistant, PKCE, refresh et stockage chiffré intégrés. Action humaine restante: créer/fournir GOOGLE_OAUTH_CLIENT_ID + GOOGLE_OAUTH_CLIENT_SECRET puis exécuter les probes réels Gmail/Calendar/Tasks avant DONE_VERIFIED.', 'P1'),
    item('GEN2-34', 'Outlook / Microsoft', 'BLOCKED_HUMAN', 'Socle OAuth Microsoft ajouté sur le runtime générique: endpoint common v2, PKCE, Mail.Read/Mail.ReadWrite/Mail.Send et offline_access d autorisation. Action humaine restante: enregistrer l application Microsoft, fournir MICROSOFT_OAUTH_CLIENT_ID + MICROSOFT_OAUTH_CLIENT_SECRET puis exécuter les probes réels Outlook avant DONE_VERIFIED.', 'P2'),
    item('GEN2-35', 'OneDrive / SharePoint', 'BLOCKED_HUMAN', 'Socle OAuth Microsoft Graph ajouté pour OneDrive et SharePoint: PKCE, stockage chiffré partagé, Files.Read.All/Sites.Read.All par défaut et scopes écriture optionnels explicitement bornés. Action humaine restante: fournir l application Microsoft réelle/consentements puis exécuter les probes Graph OneDrive et SharePoint avant DONE_VERIFIED.', 'P1'),
    ...
    item('MEL-CONN-03', 'Mail multi-fournisseurs Yahoo/Ymail + Roundcube/IMAP-SMTP', 'BLOCKED_HUMAN', 'Socle code ajouté: OAuth Yahoo/Ymail avec PKCE et scopes mail-r/mail-w, plus manifeste générique IMAP/SMTP destiné aux serveurs derrière Roundcube. Restent les identifiants Yahoo Developer et les paramètres réels IMAP/SMTP/TLS du compte Roundcube, puis les probes bout-en-bout avant DONE_VERIFIED.', 'P1'),
  ...
  phase('P09', 'Voix, avatar et multimodal', [
    item('GEN2-21', 'Images / vision', 'DONE_VERIFIED', 'Runtime image/vision provider-neutral fusionné par PR #319: analyse, traitement et génération unifiés derrière media/images, routage free-first, fallback providers, provenance checksum/prompt et garde explicite des coûts payants. Inclus dans le main exact déployé. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P1'),
    item('GEN2-22', 'Audio / transcription / voix', 'DONE', 'Runtime audio/music provider-neutral fusionné via #457/#462: transcribe/synthesize/analyze/generate, routage free-first, approbation explicite des appels payants, garde budget et provenance bornée sans octets audio bruts. Gate gen2-22-audio-runtime-ci 36230553470 vert. Validation providers réels et appareil réel requises avant DONE_VERIFIED.', 'P0'),
    item('GEN2-23', 'Vidéo', 'DONE_VERIFIED', 'Runtime vidéo provider-neutral fusionné par PR #334: analyse/traitement/génération unifiés, routage free-first, fallback providers, provenance et mode talking-avatar avec références image/audio. Inclus dans le main exact déployé. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P1'),
    ...
    item('GEN2-24', 'Documents', 'DONE_VERIFIED', 'Certification réelle acquise le 26/09/2026 sur PC Windows physique DESKTOP-0S87KA9 avec le runtime src/media/document-runtime.js: corpus local réel hétérogène PDF + DOCX + CSV + TXT, extraction et indexation réussies sur 4/4 fichiers, 14 chunks produits au total, code retour 0. Preuve sanitizée sans noms/contenus privés: proofs/gen2-24-real-corpus-20260926.json. CI gen2-24-documents-ci déjà verte.', 'P1'),
    item('MEL-VOICE-01', 'Réveil Bonjour MEL / Allô MEL', 'DONE_VERIFIED', 'Validation navigateur réelle multi-session acquise le 26/09/2026 avec deux profils Chrome 154 distincts sur Windows. Session 1: Allô MEL + commande inline => queue voice-wake-word correcte. Session 2: BONJOUR Mél arme sans envoyer, la commande suivante est acceptée, puis une commande après 8,2 s n est plus transmise. Runtime observé en fr-FR, continuous=true, interimResults=false. Les événements SpeechRecognition ont été injectés dans Chrome sans ouvrir de micro réel, conformément au contrat wake-word sans prompt micro.', 'P1'),
    ...
    item('MEL-AVATAR-02', 'Talking avatar lip-sync', 'DONE_VERIFIED', 'Validation navigateur réelle acquise le 26/09/2026 avec Chrome 154 sur Windows à partir des sources canoniques avatar-motion/lipsync et de l asset MEL réel. En état speaking, le fallback zéro coût a produit et rendu les niveaux de bouche 1, 2 et 3 avec source zero-cost-fallback; retour idle => niveau 0/source stopped. Le mode prefers-reduced-motion masque la bouche animée et supprime animation/transition. Image canonique chargée avec dimensions naturelles 1254x1254.', 'P2'),
  ...
  phase('P10', 'Téléphone, PC et appareils', [
    ...
    item('MEL-DEVICE-01', 'Ouvrir/fermer applications et fichiers', 'DONE_VERIFIED', 'Validation PC Windows réelle acquise le 26/09/2026: stress live run 36260619763 a exécuté app.open/app.close sur Notepad, file.open/file.close sur un fichier local autorisé, captures répétées et refus attendu d une application hors allowlist; 42/42 commandes terminales et 0 échec d invariant. Gate mel-device-01-files-apps-ci redevenue verte sur le correctif Windows 50b60689631da67e7633f7937dcde61a48ead8d7, fusionné via PR #505.', 'P1'),
    ...
    item('MEL-DEVICE-03', 'Wake-on-LAN séparé', 'DONE_VERIFIED', 'Module réseau Wake-on-LAN indépendant livré par PR #44: magic packet standard, validation MAC/broadcast/ports, permissions et approbation exacte, owner_halt prioritaire, transport via adaptateur compagnon injecté et audit. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P3')
  ]),
  ...
  phase('P12', 'Résilience, sauvegarde et indépendance', [
    item('GEN2-47', 'Backups / export système', 'DONE_VERIFIED', 'Certification production acquise le 26/09/2026 sur l’implémentation système D1/R2 au déploiement exact SHA f33f161dd4b1639e5b32490bfe4dfba05b5c370c (run 36233051273): backup frais system-20260926093342886 créé et vérifié, restore dry-run isolé validé, deployed_sha et backup_deployed_sha identiques au SHA déployé, sha_matches=true; le workflow a confirmé LATEST_SYSTEM_BACKUP_RESTORE_VERIFIED.', 'P1'),
    ...
    item('GEN2-49', 'Portabilité système provider-neutral', 'DONE_VERIFIED', 'Validation provider réelle acquise le 26/09/2026: le restore canonique enregistre désormais la provenance du provider sans lui déléguer d appels réseau. CI gen2-49-portability-restore-ci 36270368141 verte. Un bundle canonique à 5 contrats a été généré par le run 36270637075, transféré dans Google Drive/Checkpoints, relu depuis Drive avec archive SHA-256 a85ce35d1d10754a6d02b648687356a5e88226fdeff91f08f5d965e9ba3a3edd, puis restauré depuis cette copie avec provider=google-drive, 5/5 contrats restaurés, provider_calls=0 et external_side_effects=0.', 'P1'),
    ...
    item('MEL-RES-04', 'Cold standby autorisé', 'DONE_VERIFIED', 'Validation live acquise le 26/09/2026 sur une vraie destination froide Google Drive: bundle de reprise déjà chiffré AES-GCM-256 copié dans MELITURGOS/Checkpoints, relu depuis Drive puis matérialisé localement pour restauration sans déploiement, DNS ni bascule trafic. Source, readback et restore material font tous 668 octets et partagent le SHA-256 1701b7f14e7b93573b172a56f8e76232ffb5bd2b0815d13739e97b03752dd7e6.', 'P2'),
  ...
  phase('P15', 'Release, migration et maturité finale', [
    ...
    item('GEN2-62', 'human-actions-required', 'DONE_VERIFIED', 'Registre exact des actions humaines livré par PR #284 puis durci par PR #348: dérivation stricte depuis BLOCKED_HUMAN, aucun ghost blocker, provenance révision/fingerprint et validation fail-closed. Maintenir la cohérence avec le registre canonique. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P1'),
    item('GEN2-63', 'Règle NON-IDLE / continue-when-blocked', 'DONE_VERIFIED', 'Politique exécutable fusionnée par PR #404: tant qu un item actionable existe, les blockers humains/externes ne peuvent pas forcer IDLE; sélection déterministe issue de la Completion Matrix et validation fail-closed. Gate gen2-63-non-idle-ci 36226797726 vert. Certification finale: présent sur le SHA exact 02709095088bf71567350b1b4f4465231440d448, full-suite CI verte avant fusion et déploiement Cloudflare release 36232449914 vert avec preuve identite SHA, autonomie production et HTTP final.', 'P1'),
