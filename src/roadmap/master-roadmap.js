...
export const MASTER_ROADMAP = Object.freeze([
  ...
  phase('P04', 'Capability Bus, outils et accès au code', [
    ...
    item('GEN2-15', 'Plugin SDK', 'PARTIAL', 'SDK/runtime + registre D1 + loader serveur désormais reliés: installCandidate persiste la version, load exige plugin_id+version épinglés, résout l’artefact via un resolver injecté sans fetch arbitraire, vérifie le manifest exact puis active via les preuves durables; execute/health/disable réconcilient runtime et registre. Reste la restauration au démarrage d’une version déjà ACTIVE et un rollback durable explicite avant DONE.', 'P1'),
