...
export const MASTER_ROADMAP = Object.freeze([
  ...
  phase('P03', 'Mémoire personnelle et connaissance', [
    ...
    item('MEL-MEM-03', 'Export mémoire portable et lisible', 'DONE', 'Export portable déjà complet: format meliturgos-memory-export, schema_version 1.0.0, SHA-256 par mémoire + payload global, tri déterministe, vérification de corruption, sérialisation et parse fail-closed. tests/memory/portable-export.test.mjs passe dans les runs full-candidate 36112226371, 36112442755 et 36112691582; garder DONE tant que la CI globale reste rouge sur les deux tests UI hors de ce lot.', 'P1'),
  ...
  phase('P04', 'Capability Bus, outils et accès au code', [
    ...
    item('GEN2-15', 'Plugin SDK', 'PARTIAL', 'Le bootstrap Gen2 délègue désormais runtime.plugins au PluginRuntime canonique; le chemin durable utilise registre D1 + loader lorsque MEL_PLUGIN_RESOLVER est injecté, et les plugins chargés/rollbackés sont exposés ou remplacés dans le même CapabilityBus. Les plugins inline de compatibilité passent aussi par PluginRuntime sans persistance. Reste un resolver d’artefacts serveur concret/configuré en production et la preuve preview/live avant DONE.', 'P1'),
  ...
  phase('P06', 'Module Lab, évolution et apprentissage', [
    ...
    item('GEN2-18', 'Self Healing contrôlé', 'PARTIAL', 'Coordinateur persistant désormais exposé dans le CapabilityBus: policy.preview sans mutation, inspect seulement avec détecteur/diagnostic serveur, prepare/apply/rollback derrière approvals explicites du bus en plus de la policy GEN2-18; owner halt garde priorité, production exige l’approbation incident+repair exacte et rollback une approbation incident-scoped. Sans adapters MEL_SELF_HEALING_ADAPTERS, les capacités mutantes restent UNAVAILABLE/fail-closed. Reste à configurer et prouver des adapters réels candidate/rollback avant DONE.', 'P2'),
  ...
  phase('P07', 'Work, agents et automatisations', [
    ...
    item('GEN2-39', 'Agents / automations', 'PARTIAL', 'La façade createGen2Runtime().agents n’utilise plus de Map parallèle: registre agent D1/in-memory canonique + WorkAgentRuntime, Work DAG persistant, TASK/AUGMENTIO via le même CapabilityBus/Council, réservation atomique de lots MULTI_AI_PROTOCOL et claims automation D1 idempotents avec tiers de permissions. Compatibilité register(id,steps)/run(id) conservée. Reste à exposer les capacités agent/automation sous approval gates et relier scheduler CONDITION/EVENT/SCHEDULED au runner avant DONE.', 'P1'),
    item('GEN2-40', 'Event Bus idempotent / follow-ups', 'PLANNED', 'Transporter événements et relances idempotentes entre services', 'P1'),
    item('GEN2-41', 'Notifications', 'PARTIAL', 'Le cœur provider-neutral existant est désormais durable et branché runtime: abonnements + claims de livraison persistants D1, déduplication inter-instances, endpoints/keys redacted des lectures publiques, capacités notifications.status/list/subscribe/unsubscribe/send dans CapabilityBus avec permissions et approval gates sur mutations/envoi. Les transports WEB_PUSH/COMPANION restent injectés et un transport absent échoue explicitement sans faux succès. Reste à configurer/prouver les transports réels Web Push et compagnons avant DONE.', 'P2'),
  ...
  phase('P08', 'Connecteurs et web', [
    ...
    item('GEN2-37', 'Web / recherche', 'PARTIAL', 'Recherche sourcée durcie: domaines désormais validés et réellement contraignants sur seeds/résultats, plus d’appel arbitraire https://<domain>/search; chaque source expose classe de preuve DIRECT_CONTENT/DISCOVERY_ONLY, provenance avec SHA-256 et redirections, les seeds ne prétendent plus à une autorité vérifiée, et un DNS resolver optionnel peut bloquer rebinding/private resolution à chaque hop (mode MEL_WEB_REQUIRE_DNS_VALIDATION fail-closed). Reste preuve live production avec resolver DNS configuré et critères qualité sur sources réelles avant DONE.', 'P1'),
  ...
  phase('P09', 'Voix, avatar et multimodal', [
    ...
    item('GEN2-24', 'Documents', 'PARTIAL', 'Pipeline document canonique ajouté: détection MIME/extension/signature, limites bytes/texte/pages, UTF-8 strict, extraction intégrée TEXT/Markdown/CSV/JSON/HTML, hash SHA-256 contenu+texte, chunks déterministes, et CapabilityBus documents.status/extract/index relié aux assets D1+R2. PDF/DOCX/DOC exigent un extractor serveur injecté et son résultat est revalidé; aucun faux parsing binaire. Reste à configurer/prouver de vrais extractors PDF/DOCX/DOC (et OCR pour scans) avant DONE.', 'P1'),
  ...
  phase('P11', 'Sécurité, audit et gouvernance', [
    ...
    item('MEL-SEC-03', 'Supply-chain / dépendances / CI', 'DONE', 'Gate runtime supply-chain fail-closed prouvé sur full-candidate-ci 36113792416: npm audit vérifié, politique high/critical=0, lockfile/intégrité/provenance contrôlés, 10/10 tests supply-chain verts et runtime-sbom uploadé comme artifact 10853529180. Le run global reste rouge uniquement sur les deux tests UI hors de ce lot; conserver DONE jusqu’à une full suite globale verte avant DONE_VERIFIED.', 'P1'),
