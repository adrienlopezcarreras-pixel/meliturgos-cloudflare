...
export const MASTER_ROADMAP = Object.freeze([
  ...
  phase('P06', 'Module Lab, évolution et apprentissage', [
    ...
    item('GEN2-18', 'Self Healing contrôlé', 'PARTIAL', 'Policy fail-closed déjà complétée par un coordinateur persistant sur le DevJobRepository canonique: détection+preuves, diagnostic, candidate, tests, application bornée, owner halt prioritaire, production avec approbation incident+repair exacte et rollback toujours approuvé. Aucun adaptateur de mutation n’est inventé: absence = 503 fail-closed. Reste à brancher des adapters réels de candidate/rollback et exposer la capacité runtime sous les approval gates avant DONE.', 'P2'),
  ...
  phase('P11', 'Sécurité, audit et gouvernance', [
    ...
    item('MEL-SEC-03', 'Supply-chain / dépendances / CI', 'PARTIAL', 'SBOM runtime déterministe + vérification lockfile/intégrité/provenance existent; le gate CI est désormais strictement fail-closed si npm audit est indisponible/non vérifiable, applique la politique high/critical=0 et produit runtime-sbom.json uploadé comme preuve. Reste à obtenir un run full-candidate où ce step s’exécute après résolution des échecs UI hors de ce lot avant DONE_VERIFIED.', 'P1'),
