...
export function validate(value, schema) {
  ...
  const supported = ['type','properties','required','additionalProperties','items','enum','minLength','maxLength','minItems','maxItems','minimum','maximum','description'];
  ...
  if (schema.type === 'array') {
    requireValue(Boolean(schema.items), 'ITEM_SCHEMA_REQUIRED');
    requireValue(value.length >= (schema.minItems ?? 0) && value.length <= (schema.maxItems ?? 1000), 'INVALID_ITEMS_LENGTH');
