export const CAPABILITY_WATCH_SCHEMA = 'mel.capability-watch.v1';
...
export async function runCapabilityWatch({
  ...
  if (!plan.due) {
    return {
      ...
      next_due_at: plan.next_window_at,
      state: plan.previous,
      results: [],
      regressions: [],
      overall: null,
  ...
  if (typeof evaluator !== 'function') {
    return {
      ...
      reason: plan.reason,
      measured_at: measuredAt,
      next_due_at: plan.window_started_at,
      state: plan.previous,
      results: [],
      regressions: [],
      overall: null,
  ...
  for (const target of normalizedTargets) {
    ...
    if (!Number.isFinite(Number(rawScore))) {
      return {
        ...
        capability_id: target.id,
        failure: `CAPABILITY_WATCH_SCORE_REQUIRED:${target.id}`,
        measured_at: measuredAt,
        next_due_at: plan.window_started_at,
        state: plan.previous,
        results,
  ...
  const lastScores = Object.fromEntries(results.map(result => [result.id, result.score]));
  const nextState = {
    schema: CAPABILITY_WATCH_SCHEMA,
  ...
  return {
    ...
    measured_at: measuredAt,
    next_due_at: plan.next_window_at,
    source_sha: nextState.last_source_sha,
    interval_ms: plan.interval_ms,
    regression_threshold: threshold,
    overall: scoreRun(results),
    results,
    regressions,
    state: nextState,
  };
