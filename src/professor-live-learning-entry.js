import app from './professor-validated-features-entry.js';
...
(()=>{
  if(window.__melProfessorLiveLearning)return;window.__melProfessorLiveLearning=true;
  let timer=null;
  ...
  function ensureDetailRows(){
    ...
    const rows=[['Leçons projet','learnProjectExperience'],['Source XP','learnXpSource'],['XP observée','learnObservedXp'],['Dernière mesure','learnMeasuredAt']];
  ...
  async function refresh(){
    try{
      ...
      if(meta&&!meta.textContent.includes('live indisponible'))meta.textContent+=' · live indisponible';
    }
  }
  function start(){
    ensureDetailRows();refresh();
