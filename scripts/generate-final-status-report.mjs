...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === '--json') options.format = 'json';
      ...
      if (!next || next.startsWith('-')) throw new Error('FINAL_STATUS_REPORT_OUTPUT_PATH_REQUIRED');
      options.output = next;
      index += 1;
    } else if (arg === '--help' || arg === '-h') {
      options.help = true;
    } else {
      throw new Error(`FINAL_STATUS_REPORT_UNKNOWN_ARGUMENT:${arg}`);
    }
  }
  return options;
}
...
function help() {
  return [
    'Usage: node scripts/generate-final-status-report.mjs [options]',
    '',
    'Options:',
...
const report = generateFinalStatusReport({
  generatedAt: options.timestamp ? new Date() : null,
});
const rendered = options.format === 'json'
  ? finalStatusReportToJson(report)
  : finalStatusReportToMarkdown(report);
