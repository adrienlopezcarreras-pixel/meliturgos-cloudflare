...
test('full mode exposes one canonical IA salon and one composer', async () => {
  ...
  assert.equal(occurrences(html, 'id="roomLog"'), 1);
  assert.equal(occurrences(html, 'id="roomInput"'), 1);
  assert.equal(occurrences(html, 'id="roomSend"'), 1);
  assert.equal(occurrences(html, 'id="roomTarget"'), 1);
...
test('canonical Mentor uses guarded zero-euro routes', async () => {
