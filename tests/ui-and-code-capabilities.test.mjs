...
test('full mode keeps MEL responsive and provides fast verified reading controls', async () => {
  ...
  assert.match(page, /Salon Adrien · MEL · Mentor gratuit/);
  assert.match(page, /Zero-Euro Governor/);
  assert.match(page, /Mentor gratuit/);
  ...
  assert.match(page, /mel\.full\.last\.panel\.v2/);
  assert.match(page, /F5 revient au dernier onglet/);
