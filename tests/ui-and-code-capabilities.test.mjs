...
test('code questions are routed by the native chat capability path', () => {
  ...
  assert.deepEqual(inferNativeCodeCapability('Cherche dans ton code createDefaultCapabilityBus'), { id: 'code.search', input: { query: 'dans ton code createDefaultCapabilityBus' } });
});

test('explicit code file requests are routed to code.read', () => {
  assert.deepEqual(inferNativeCodeCapability('Lis le fichier src/router.js'), { id: 'code.read', input: { path: 'src/router.js' } });
...
test('full mode is unified, responsive and evidence-based', async () => {
  ...
  assert.match(page, /Adrien · MEL · Mentor · Conseil/);
  ...
  assert.match(page, /\/assets\/avatars\/mel-classic\.webp/);
  assert.doesNotMatch(page, /Statut MEL|Prochaine tâche|Fin du chat|Grande lecture|meliturgos-avatar-fille\.png/);
