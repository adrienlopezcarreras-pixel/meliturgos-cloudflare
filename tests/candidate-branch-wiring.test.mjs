...
test('candidate deployment reads and develops the canonical candidate branch', () => {
  const config = loadWrangler();
  assert.equal(config.vars.MEL_GITHUB_BRANCH, 'candidate/augmentio-core');
  assert.equal(config.vars.MEL_TEACHER_BRANCH, 'candidate/augmentio-core');
  assert.match(config.vars.MEL_GITHUB_REPOSITORY, /meliturgos-cloudflare$/);
});

test('capability bus safe defaults also remain on the canonical candidate when bindings are absent', () => {
  const state = setDefaultCapabilityEnvironment({});
  assert.equal(state.github_branch, 'candidate/augmentio-core');
  assert.equal(state.teacher_branch, 'candidate/augmentio-core');
...
test('runtime autonomy and Teacher transports never silently point back to superseded branches', () => {
  for (const relative of runtimeBranchSources) {
    ...
    assert.match(source, /candidate\/augmentio-core/, relative);
  }
});
