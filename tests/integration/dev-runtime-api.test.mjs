...
test('Professor job lifecycle reaches explicit approval through bridge API', async () => {
  let r = await devRuntime(new Request('http://x/api/professor/dev/jobs', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ goal: 'safe fixture change' }) }), env);
  ...
  r = await devRuntime(new Request('http://x/api/dev-bridge/result', { method: 'POST', headers: auth, body: JSON.stringify({ job_id: j.job_id, status: 'READY_FOR_REVIEW', tests: [{ passed: true }], diff_summary: 'fixture' }) }), env);
  assert.equal((await r.json()).status, 'READY_FOR_REVIEW');
  r = await devRuntime(new Request(`http://x/api/professor/dev/jobs/${j.job_id}/approve`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' }), env);
...
test('failed tests persist REPAIR_REQUIRED and only a passing retest can restore READY_FOR_REVIEW', async () => {
  let r = await devRuntime(new Request('http://x/api/professor/dev/jobs', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ goal: 'repair fixture change' }) }), env);
  const j = await r.json();
  r = await devRuntime(new Request('http://x/api/dev-bridge/result', { method: 'POST', headers: auth, body: JSON.stringify({ job_id: j.job_id, status: 'READY_FOR_REVIEW', tests: [{ name: 'targeted', passed: false, exit_code: 1 }], diff_summary: 'candidate needs repair' }) }), env);
  ...
  await assert.rejects(() => devRuntime(new Request(`http://x/api/professor/dev/jobs/${j.job_id}/approve`, { method: 'POST', headers: { 'content-type': 'application/json' }, body: '{}' }), env), (error) => error?.code === 'JOB_NOT_READY');
  r = await devRuntime(new Request('http://x/api/dev-bridge/result', { method: 'POST', headers: auth, body: JSON.stringify({ job_id: j.job_id, status: 'READY_FOR_REVIEW', tests: [{ name: 'targeted', passed: true, exit_code: 0 }], diff_summary: 'candidate repaired and retested' }) }), env);
