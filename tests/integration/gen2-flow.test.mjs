...
const councilReport = { status: 'COMPLETE', phase: 'STATE_OF_PLAY_BEFORE_DEVELOPMENT', responses: [{ member: 'fixture-a', answer: 'reuse' }, { member: 'fixture-b', answer: 'test' }] };

test('MEL chat archives before retrieval, injects cognitive memory, routes through ModelRouter, and archives response', async () => {
  ...
  try {
    ...
    assert.ok(last[0].content.includes('MÉMOIRE COGNITIVE'));
...
test('chat capability path crosses CapabilityBus and injects a trusted tool result into model context', async () => {
