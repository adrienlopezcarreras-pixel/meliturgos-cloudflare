import test from 'node:test';
import assert from 'node:assert/strict';
import { devRuntime } from '../../src/dev/runtime-api.js';
import { LocalDevBridge } from '../../src/dev/dev-bridge.js';
import { D1DevJobRepository } from '../../src/dev/d1-dev-job-repository.js';

test('Professor → Worker job → CapabilityBus bridge → approval → candidate commit without touching the outer branch', async()=>{
  const env={MEL_DEV_BRIDGE_TOKEN:'e2e-token'}, professor={headers:{'content-type':'application/json'}};
  const repository=new D1DevJobRepository(null,{memoryStore:new Map()});
  let r=await devRuntime(new Request('http://x/api/professor/dev/jobs',{method:'POST',headers:professor,body:JSON.stringify({goal:'safe candidate fixture change'})}),env,{repository});
  const created=await r.json();
  const b=new LocalDevBridge({repoRoot:process.cwd(),candidateRoot:`/tmp/mel-self-dev-e2e-${crypto.randomUUID()}`});
  const ctx={owner:'bridge',permissions:[],requestId:crypto.randomUUID()};
  assert.ok(b.bus.describe('code.read'));
  assert.ok((await b.bus.execute('code.read',{path:'package.json'},ctx)).content.includes('mel:dev-bridge'));
  r=await devRuntime(new Request('http://x/api/dev-bridge/claim',{method:'POST',headers:{authorization:'Bearer e2e-token'},body:'{}'}),env,{repository});
  const job=await r.json();
  const outerBranch=await b.currentBranch();
  const c=await b.bus.execute('dev.create_candidate',{job_id:job.job_id},ctx);
  await b.bus.execute('dev.apply_change',{job_id:job.job_id,path:'fixture.txt',content:'candidate\n'},ctx);
  const check=await b.exec(['git','diff','--check'],b.candidate(job.job_id).dir); assert.equal(check.exit_code,0);
  await devRuntime(new Request('http://x/api/dev-bridge/result',{method:'POST',headers:{authorization:'Bearer e2e-token','content-type':'application/json'},body:JSON.stringify({job_id:created.job_id,status:'READY_FOR_REVIEW',candidate_branch:c.branch,files_changed:['fixture.txt'],tests:[{passed:true}],diff_summary:'fixture'})}),env,{repository});
  r=await devRuntime(new Request(`http://x/api/professor/dev/jobs/${created.job_id}/approve`,{method:'POST',headers:professor,body:'{}'}),env,{repository}); assert.equal((await r.json()).status,'APPROVED');
  const committed=await b.bus.execute('dev.commit',{job_id:job.job_id,files:['fixture.txt'],message:'safe candidate',approved:true},ctx); assert.equal(committed.status,'COMMITTED');
  assert.equal(await b.currentBranch(),outerBranch);
  assert.ok(outerBranch);
  await b.rollbackCandidate(job.job_id);
});
