...
test('GEN2-53 release discipline is durable and available to MEL training', async () => {
  const lesson = BOOTSTRAP_CORRECTIONS.find(row => row.id === 'bootstrap-gen2-53-release-discipline-20260916');
  assert.ok(lesson);
  assert.equal(lesson.validated, true);
  assert.equal(lesson.quality, 1);
  assert.ok(lesson.tags.includes('canary'));
  assert.ok(lesson.tags.includes('rollback'));
  assert.match(lesson.after, /SHA exact/);
  assert.match(lesson.after, /sans trafic/);
  assert.match(lesson.after, /stable@100\/candidate@0/);
  assert.match(lesson.after, /rollback/);
  assert.match(lesson.after, /smoke live post-déploiement/);
