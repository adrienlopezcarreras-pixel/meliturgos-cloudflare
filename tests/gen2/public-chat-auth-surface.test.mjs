...
test('MEL-CONN-01 public chat API methods are explicit sanitized exceptions', () => {
  assert.equal(classifyHttpAuthSurface(new Request('https://mel.example/api/public/wordpress/chat',{method:'POST'})).kind,'PUBLIC_SANITIZED');
  assert.equal(classifyHttpAuthSurface(new Request('https://mel.example/api/public/wordpress/chat',{method:'OPTIONS'})).kind,'PUBLIC_SANITIZED');
  assert.equal(classifyHttpAuthSurface(new Request('https://mel.example/api/public/wordpress/chat',{method:'GET'})).kind,'OWNER_AUTH');
  assert.equal(classifyHttpAuthSurface(new Request('https://mel.example/api/public/wordpress/other',{method:'POST'})).kind,'OWNER_AUTH');
});
