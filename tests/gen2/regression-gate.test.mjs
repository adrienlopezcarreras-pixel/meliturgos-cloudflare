...
test('MEL-EVAL-03 rejects malformed policy instead of silently weakening the gate', () => {
  assert.throws(
  ...
  assert.throws(
    () => normalizeRegressionPolicy({ require_evidence: 'false' }),
    (error) => error?.code === 'REGRESSION_REQUIRE_EVIDENCE_INVALID',
  );
});
