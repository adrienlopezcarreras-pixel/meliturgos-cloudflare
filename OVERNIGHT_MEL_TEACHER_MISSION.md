...
- Add a permissioned Flipper Zero adapter as a hardware bridge for owner-authorized device automation and diagnostics. Initial safe targets include infrared send/control for owned TVs/appliances, NFC/RFID inspection for tags the owner is authorized to test, and Sub-GHz inspection/automation only where the protocol and device ownership/authorization are established.
- The Flipper Zero integration must never be used to bypass authentication, clone third-party access credentials, replay protected rolling codes, unlock vehicles/doors/alarms without authorization, or weaken security controls. Unsupported or ambiguous operations fail closed and remain BLOCKED_EXTERNAL or require explicit owner confirmation.
