...
static int subscribe_complete(uint16_t conn_handle, const struct ble_gatt_error *error,
                              struct ble_gatt_attr *attr, void *arg) {
    (void)attr; (void)arg;
    if (conn_handle != g_conn_handle) return 0;
    if (error && error->status != 0) {
        ESP_LOGW(TAG, "MEL Mobile subscribe confirmation failed status=%d", error->status);
        g_ready.store(false);
...
static void on_discovery_complete(const struct peer *peer, int status, void *arg) {
    ...
    ESP_LOGI(TAG, "MEL Mobile subscription requested; waiting for confirmation");
}
